...
Event::Event(const char* _name, const BulgarianDate& _date, const Time& _startTime, const Time& _endTime)
    : date(_date), startTime(_startTime), endTime(_endTime)
{
    setName(_name);
    validateTime();
}

Event::Event(const char* _name, unsigned day, unsigned month, unsigned year,
    unsigned startHour, unsigned startMinutes, unsigned startSeconds,
    unsigned endHour, unsigned endMinutes, unsigned endSeconds)
    : date(day, month, year), startTime(startHour, startMinutes, startSeconds),
